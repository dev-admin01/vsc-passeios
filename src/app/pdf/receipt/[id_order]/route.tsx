...
export async function GET(
  ...
  const response = await fetch(
    `${baseUrl}/api/receipt/public/pdf/${id_order}`,
    {}
  );
  ...
  if (!pdfData.orderPdfData || !pdfData.orderPdfData.id_status_order) {
    return new NextResponse("Orçamento não encontrado.", { status: 404 });
  }

  if (pdfData.orderPdfData.id_status_order < 9) {
    return new NextResponse("Orçamento não encontrado.", { status: 404 });
  }

  // Passe os dados que precisar para o componente do PDF
  const docElement = <ReceiptPDF pdfData={pdfData} />;
